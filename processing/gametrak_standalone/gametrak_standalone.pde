 ...
 *
 * <p>This Processing sketch talks to the GameTrak directly through the
 * GameTrakDirectHid Java helper in this sketch folder. The .pde file stays
 * intentionally simple so Processing's preprocessor does not have to parse
 * hid4java types.</p>
 */
 ...
 */
public void setup() {
  surface.setTitle("GameTrak Standalone HID");
  textFont(createFont("Helvetica", 16));
  frameRate(60);
  registerMethod("dispose", this);

  gameTrak = new GameTrakDirectHid();
  gameTrak.start();
}
...
void drawHeader(String currentStatus, String currentDeviceSummary, int reports, long lastSeen) {
  ...
  text("GameTrak Standalone HID", 40, 48);
